...
-   Cree una carpeta en su computador con "Control de Cambios" usando Git
...
La hipÓtesis nula (HO):La facilitación interespecífica no tiene efectos significativos en el establecimiento de especies raras ni en la reducción de las disparidades de rendimiento entre los adultos.

La hipótesis (Ha):La facilitación interespecífica favorece el establecimiento de especies raras y reduce las disparidades de rendimiento entre los adultos.
...
library(agricolae)
sitios <-c("Parque las islas", "Comunidad Muyuna", "Colonso Chalupas","Herbario de UEA")
r <- 3
seed = 100
...
crd_re <- crd1$book
crd_re$Etapas <- NaN
crd_re$Temperatura<- NaN
crd_re$Abundancia<- NaN
crd_re$Distribución<- NaN
```

Explique el resultado muestra:

Los resultados muestran la generación de un diseño experimental CRD con 3 repeticiones y 2 bloques, utilizando la función design.crd() del paquete agricolae. Posteriormente, se realizan modificaciones en la copia del diseño experimental, asignando el valor NaN a ciertas columnas.

## Variables

¿Cuáles son sus variables (dependiente(s) e independiente(s))? Explique cada una de las variables Variable independiente:

Sitios: Esta variable representa los diferentes tratamientos o sitios del experimento. En este caso, los nombres de los sitios son "Parque las islas", "Comunidad Muyuna", "Colonso Chalupas" y "Herbario de UEA". Es una variable categórica que define las diferentes condiciones o grupos que se están comparando en el experimento.

"Etapas", "Temperatura", "Abundancia" y "Distribución": estas son como variables dependientes: que se mencionan más adelante podrían ser consideradas como variables dependientes en un contexto más amplio. Estas variables podrían representar medidas o características que se están evaluando en relación con los diferentes tratamientos o sitios del experimento.
...
![](images/Grafica%20e%20hip%C3%B3tesis.jpg)
